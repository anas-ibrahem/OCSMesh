...
Usage:
    ...
    PYTHONPATH=. python tests/api/manual_mpi_test.py

    # MPI test with 2 ranks
    PYTHONPATH=. mpirun -n 2 python tests/api/manual_mpi_test.py

    # MPI test with 4 ranks
    PYTHONPATH=. mpirun -n 4 python tests/api/manual_mpi_test.py
"""
...
# Approach 2: rank 0 is a dedicated coordinator; workers block in the
# point-to-point mpi_worker_loop() until the coordinator streams them
# tasks and finally sends TAG_STOP.
...
def main():
    ...
    if rank == 0:
        ...
        if size > 1:
            ...
            # Rank 0 (dedicated coordinator) calls meshdata() normally.
            # Internally it streams tasks to workers over send()/recv()
            # and releases them with TAG_STOP when done.
            #
            # Coordinator error policy: if a task fails, meshdata() raises
            # AFTER releasing the workers. But the workers then proceed to
            # the Barrier below while rank 0 would skip it — so on any
            # unrecoverable error we Abort the whole job to avoid a hang.
            try:
                meshdata_mpi = hfun_mpi.meshdata()
            except Exception as exc:  # pragma: no cover - failure path
                log(f"MPI meshdata() failed: {exc!r} -> aborting job")
                if comm is not None:
                    comm.Abort(1)
                raise
        ...
        # ── Approach 2: workers run the point-to-point loop ──
        # The worker blocks in recv() until the coordinator (rank 0)
        # sends it a task, computes it, returns the result, and repeats
        # until it receives TAG_STOP. No manual dispatch, no collective
        # ordering to get wrong.
        log("Entering mpi_worker_loop()...")
        mpi_worker_loop()
        log("mpi_worker_loop() returned.")
