...
# ── NEW import: mpi_worker_loop handles all collectives via tags ──
...
def main():
    ...
    if rank == 0:
        ...
        if size > 1:
            ...
            # Rank 0 calls meshdata() normally.
            # Internally, the MPI method broadcasts tags so workers
            # in mpi_worker_loop() know when to participate.
        ...
        # ── NEW: Workers use mpi_worker_loop() ──
        # No need to manually call _mpi_dispatch — the loop handles
        # all collective operations automatically via tag broadcasts
        # from Rank 0.
        log("Entering mpi_worker_loop()...")
        mpi_worker_loop()
        log("mpi_worker_loop() returned.")

        # ── OLD approach (fragile, required knowing internal details) ──
