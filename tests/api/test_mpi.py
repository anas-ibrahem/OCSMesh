...
from ocsmesh.hfun.collector import (
    _mpi_dispatch, _meshdata_task_worker, mpi_worker_loop
)
...
class TestMPIWriteHfun(unittest.TestCase):
    ...
    # ────────────────────────────────────────────────────────────────
    # NEW TESTS — workers use mpi_worker_loop() (clean API)
    #
    # Rank 0 calls normal methods (hfun.meshdata()). Internally,
    # the MPI method broadcasts a tag before each collective.
    # Workers in mpi_worker_loop() receive the tag and participate.
    # No manual _mpi_dispatch calls needed — no deadlock risk.
    # ────────────────────────────────────────────────────────────────
