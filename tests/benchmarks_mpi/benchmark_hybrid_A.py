...
Run (one command — tiles and cores/rank are auto-detected):
    ...
    # 16 ranks = 15 tiles + 1 coordinator; each worker rank sizes its
    # internal Pool from the cores actually available to it:
    mpiexec -n 16 python tests/benchmarks/benchmark_hybrid_A.py

    # serial_mp baseline only (no MPI):
    python tests/benchmarks/benchmark_hybrid_A.py --mode serial_mp --tiles 15
"""
...
def build_and_run(tile_paths, nprocs, execution_mode):
    ...
    stages['contours'] = time.perf_counter() - start
    stages['total'] = stages['contours']
...
def run_mpi(tile_paths, nprocs):
    """MPI path: 1 tile per rank; each rank's add_feature uses Pool(nprocs).
...
def main():
    ...
    tdir = hu.shared_tmpdir(comm, 'ocsmesh_hybridA_')
