"""Hybrid MPI + Pool e2e benchmark — Config F (all MPI-dispatched stages).

Same structure as benchmark_e2e_D.py. See that file for full docs.

Config F is specifically designed to benchmark the fully MPI-parallel pipeline
including all user-provided shapes and custom functions:
  ...
  add_patch                     YES — all ranks
  add_feature (line features)   YES — all ranks
  add_topo_bound_constraint     YES — all ranks
  add_topo_func_constraint      YES — all ranks
"""
...
CONFIG_DESC = (
    ...
    '+ patch + line features + topo-bound constraint + topo-func constraint'
)
...
def build_and_run(tile_paths, nprocs, execution_mode):
    """Build collector with config F refinements and run full msh_t().

    All steps in this config are MPI-dispatched, providing a comprehensive
    test of the entire refinement pipeline.
    """
    hfun = ocsmesh.Hfun(
    ...
    # 5. Patch (now MPI dispatched)
    hfun.add_patch(
        shape=box(0.2, 0.2, 0.6, 0.6), target_size=400, expansion_rate=0.005)

    # 6. Line features (now MPI dispatched)
    # A simple diagonal line feature across the first tile's span
    linestring_grid = LineString([(0.0, 0.0), (1.0, 1.0)])
    hfun.add_feature(
        shape=linestring_grid,
        expansion_rate=0.005, target_size=300)

    # 7. TopoConstConstraint
    hfun.add_topo_bound_constraint(
        value=800, upper_bound=0, lower_bound=-20, value_type='min')

    # 8. TopoFuncConstraint
    hfun.add_topo_func_constraint(
        func=_half_depth, upper_bound=0, lower_bound=-20, value_type='min')
...
def main():
    ...
    tdir = hu.shared_tmpdir(comm, 'ocsmesh_e2eF_')
