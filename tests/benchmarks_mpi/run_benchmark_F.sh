#!/bin/bash
# =============================================================================
#  Config F: fully MPI-dispatched pipeline (all stages)
#
...
#    - add_patch                (geometry, uses intra-rank thread pool)
#    - add_feature              (geometry, uses intra-rank thread pool)
#    - add_topo_bound_constraint (TopoConstConstraint, pure NumPy, MPI)
#    - add_topo_func_constraint  (TopoFuncConstraint, named fn, MPI)
#
#  This config exercises every single MPI-dispatched refinement capability.
#
#  ISOLATION STRATEGY:
...
BENCH="${SCRIPT_DIR}/benchmark_e2e_F.py"

# Generate timestamp for logging
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
LOG_FILE="${SCRIPT_DIR}/log_e2e_F_${TIMESTAMP}.txt"
...
echo "========================================="
echo " Config F Benchmark — ${TIMESTAMP}"
...
PYTHONWARNINGS=ignore mpiexec -n 5 python "${BENCH}" \
    ...
    --json "${SCRIPT_DIR}/results_F_correctness_${TIMESTAMP}.json"
...
PYTHONWARNINGS=ignore mpiexec -n $RANKS_15 python "${BENCH}" \
    ...
    --json "${SCRIPT_DIR}/results_F_15t_npool_${TIMESTAMP}.json"
echo "  [sleep 120s cooldown]"
sync; sleep 120
...
PYTHONWARNINGS=ignore mpiexec -n $RANKS_15 python "${BENCH}" \
    ...
    --json "${SCRIPT_DIR}/results_F_15t_hybrid_${TIMESTAMP}.json"
echo "  [sleep 120s cooldown]"
sync; sleep 120
...
PYTHONWARNINGS=ignore mpiexec -n $RANKS_HEAVY python "${BENCH}" \
    ...
    --json "${SCRIPT_DIR}/results_F_4t_npool_${TIMESTAMP}.json"
echo "  [sleep 120s cooldown]"
sync; sleep 120
...
PYTHONWARNINGS=ignore mpiexec -n $RANKS_HEAVY python "${BENCH}" \
    ...
    --json "${SCRIPT_DIR}/results_F_4t_hybrid_${TIMESTAMP}.json"
echo "  [sleep 120s cooldown]"
sync; sleep 120
...
python3 - "${SCRIPT_DIR}" "${TIMESTAMP}" "F" <<'PYEOF'
import json, sys, os
...
echo "JSON files: ${SCRIPT_DIR}/results_F_*_${TIMESTAMP}.json"
