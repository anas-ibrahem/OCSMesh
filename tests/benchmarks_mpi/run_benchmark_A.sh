...
BENCH="${SCRIPT_DIR}/benchmark_e2e_A.py"

# Generate timestamp for logging
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
LOG_FILE="${SCRIPT_DIR}/log_e2e_A_${TIMESTAMP}.txt"
...
echo "========================================="
echo " Config A Benchmark — ${TIMESTAMP}"
...
#  Fast (~20s).  Confirms raster-exact matching (OK / FAIL).
...
PYTHONWARNINGS=ignore mpiexec -n 5 python "${BENCH}" \
    ...
    --json "${SCRIPT_DIR}/results_A_correctness_${TIMESTAMP}.json"
...
PYTHONWARNINGS=ignore python "${BENCH}" \
    ...
    --json "${SCRIPT_DIR}/results_A_15t_serial_${TIMESTAMP}.json"
...
PYTHONWARNINGS=ignore mpiexec -n $RANKS_15 python "${BENCH}" \
    ...
    --json "${SCRIPT_DIR}/results_A_15t_npool_${TIMESTAMP}.json"
echo "  [sleep 120s cooldown]"
sync; sleep 120
...
PYTHONWARNINGS=ignore mpiexec -n $RANKS_15 python "${BENCH}" \
    ...
    --json "${SCRIPT_DIR}/results_A_15t_hybrid_${TIMESTAMP}.json"
echo "  [sleep 120s cooldown]"
sync; sleep 120
...
PYTHONWARNINGS=ignore python "${BENCH}" \
    ...
    --json "${SCRIPT_DIR}/results_A_4t_serial_${TIMESTAMP}.json"
echo "  [sleep 120s cooldown]"
sync; sleep 120
...
PYTHONWARNINGS=ignore mpiexec -n $RANKS_HEAVY python "${BENCH}" \
    ...
    --json "${SCRIPT_DIR}/results_A_4t_npool_${TIMESTAMP}.json"
echo "  [sleep 120s cooldown]"
sync; sleep 120
...
PYTHONWARNINGS=ignore mpiexec -n $RANKS_HEAVY python "${BENCH}" \
    ...
    --json "${SCRIPT_DIR}/results_A_4t_hybrid_${TIMESTAMP}.json"
echo "  [sleep 120s cooldown]"
sync; sleep 120
...
python3 - "${SCRIPT_DIR}" "${TIMESTAMP}" "A" <<'PYEOF'
import json, sys, os
...
def read_wall(path):
    ...
    with open(path) as f:
        d = json.load(f)
    # JSON format: {"config":..., "results": {"mode": {"wall_s": ...}}}
...
for tag, desc in variants:
    ...
    # At 1 core/rank, mpi_hybrid should match mpi_no_pool closely
...
echo "JSON files: ${SCRIPT_DIR}/results_A_*_${TIMESTAMP}.json"
