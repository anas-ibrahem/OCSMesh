...
BENCH="${SCRIPT_DIR}/benchmark_e2e_C.py"

# Generate timestamp for logging
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
LOG_FILE="${SCRIPT_DIR}/log_e2e_C_${TIMESTAMP}.txt"
...
echo "========================================="
echo " Config C Benchmark — ${TIMESTAMP}"
...
PYTHONWARNINGS=ignore mpiexec -n 5 python "${BENCH}" \
    ...
    --json "${SCRIPT_DIR}/results_C_correctness_${TIMESTAMP}.json"
...
PYTHONWARNINGS=ignore python "${BENCH}" \
    ...
    --json "${SCRIPT_DIR}/results_C_15t_serial_${TIMESTAMP}.json"
...
PYTHONWARNINGS=ignore mpiexec -n $RANKS_15 python "${BENCH}" \
    ...
    --json "${SCRIPT_DIR}/results_C_15t_npool_${TIMESTAMP}.json"
echo "  [sleep 120s cooldown]"
sync; sleep 120
...
PYTHONWARNINGS=ignore mpiexec -n $RANKS_15 python "${BENCH}" \
    ...
    --json "${SCRIPT_DIR}/results_C_15t_hybrid_${TIMESTAMP}.json"
echo "  [sleep 120s cooldown]"
sync; sleep 120
...
PYTHONWARNINGS=ignore python "${BENCH}" \
    ...
    --json "${SCRIPT_DIR}/results_C_4t_serial_${TIMESTAMP}.json"
echo "  [sleep 120s cooldown]"
sync; sleep 120
...
PYTHONWARNINGS=ignore mpiexec -n $RANKS_HEAVY python "${BENCH}" \
    ...
    --json "${SCRIPT_DIR}/results_C_4t_npool_${TIMESTAMP}.json"
echo "  [sleep 120s cooldown]"
sync; sleep 120
...
PYTHONWARNINGS=ignore mpiexec -n $RANKS_HEAVY python "${BENCH}" \
    ...
    --json "${SCRIPT_DIR}/results_C_4t_hybrid_${TIMESTAMP}.json"
echo "  [sleep 120s cooldown]"
sync; sleep 120
...
python3 - "${SCRIPT_DIR}" "${TIMESTAMP}" "C" <<'PYEOF'
import json, sys, os
...
echo "JSON files: ${SCRIPT_DIR}/results_C_*_${TIMESTAMP}.json"
