#!/bin/bash
# =============================================================================
#  Config D: full pipeline (contours + channels + flow limiter + const val
#                           + patch + topo-bound constraint)
#
#  NOTE: topo-bound constraints are NOT yet parallelized — they run serially
#  in all modes. The speedup from MPI/Pool comes from the earlier stages.
#
#  ISOLATION STRATEGY:
...
BENCH="${SCRIPT_DIR}/benchmark_e2e_D.py"

# Generate timestamp for logging
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
LOG_FILE="${SCRIPT_DIR}/log_e2e_D_${TIMESTAMP}.txt"
...
PYTHONWARNINGS=ignore mpiexec -n 5 python "${BENCH}" \
    ...
    --json "${SCRIPT_DIR}/results_D_correctness_${TIMESTAMP}.json"
...
#   Note: constraint phase remains serial — D is the worst case.
# ---------------------------------------------------------
SIZE_HEAVY=3000
TILES_HEAVY=4
RANKS_HEAVY=5
...
python3 - "${SCRIPT_DIR}" "${TIMESTAMP}" "D" <<'PYEOF'
import json, sys, os
...
echo "JSON files: ${SCRIPT_DIR}/results_D_*_${TIMESTAMP}.json"
