"""Hybrid MPI + Pool e2e benchmark — Config A (contours only).

Compares three execution strategies on a contour-only workload using
the full msh_t() pipeline:

1. serial_mp   — Serial tile loop, shared Pool for intra-tile add_feature.
...
Run (one command — tiles and cores/rank are auto-detected):

    # 16 ranks = 15 tiles + 1 coordinator:
    mpiexec -n 16 python benchmark_e2e_A.py

    # serial_mp baseline only (no MPI):
    python benchmark_e2e_A.py --mode serial_mp --tiles 15
"""
...
def build_and_run(tile_paths, nprocs, execution_mode):
    """Build collector with config A refinements and run full msh_t()."""
    hfun = ocsmesh.Hfun(
...
def main():
    ...
    tdir = hu.shared_tmpdir(comm, 'ocsmesh_e2eA_')
