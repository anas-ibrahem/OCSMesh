"""Hybrid MPI + Pool e2e benchmark — Config D (full pipeline).

Same structure as benchmark_e2e_A.py. See that file for full docs.

Adds patch + topo-bound constraint on top of Config C. This is the
heaviest config — tests the full pipeline. Constraints are NOT yet
parallelized (serial only in all modes); the speedup comes from
...
def build_and_run(tile_paths, nprocs, execution_mode):
    """Build collector with config D refinements and run full msh_t()."""
    hfun = ocsmesh.Hfun(
    ...
    # Constraints are NOT parallelized yet — serial in all modes.
...
def main():
    ...
    tdir = hu.shared_tmpdir(comm, 'ocsmesh_e2eD_')
