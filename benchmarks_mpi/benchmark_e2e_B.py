"""Hybrid MPI + Pool e2e benchmark — Config B (contours + channels).

Same structure as benchmark_e2e_A.py. See that file for full docs.

Adds add_channel() on top of Config A. Channels call add_patch →
add_feature internally, so they also benefit from the hybrid MPI + Pool
pattern.
"""
...
def build_and_run(tile_paths, nprocs, execution_mode):
    """Build collector with config B refinements and run full msh_t()."""
    hfun = ocsmesh.Hfun(
...
def main():
    ...
    tdir = hu.shared_tmpdir(comm, 'ocsmesh_e2eB_')
