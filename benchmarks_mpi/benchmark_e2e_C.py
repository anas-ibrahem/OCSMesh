"""Hybrid MPI + Pool e2e benchmark — Config C (contours + channels + flow limiter + const val).

Same structure as benchmark_e2e_A.py. See that file for full docs.

Adds flow limiter and constant value on top of Config B. Flow limiters
and const_val are parallelized via the 3-phase file-path pattern, so
they also benefit from MPI distribution.
"""
...
def build_and_run(tile_paths, nprocs, execution_mode):
    """Build collector with config C refinements and run full msh_t()."""
    hfun = ocsmesh.Hfun(
...
def main():
    ...
    tdir = hu.shared_tmpdir(comm, 'ocsmesh_e2eC_')
