...
BENCH="tests/benchmarks/benchmark_e2e_C.py"
...
PYTHONWARNINGS=ignore mpiexec -n 5 python ${BENCH} \
    ...
    --json results_C_correctness_${TIMESTAMP}.json
...
PYTHONWARNINGS=ignore python ${BENCH} \
    ...
    --json results_C_15t_serial_${TIMESTAMP}.json
sync; sleep 5
...
PYTHONWARNINGS=ignore mpiexec -n $RANKS_15 python ${BENCH} \
    ...
    --json results_C_15t_npool_${TIMESTAMP}.json
sync; sleep 5
...
PYTHONWARNINGS=ignore mpiexec -n $RANKS_15 python ${BENCH} \
    ...
    --json results_C_15t_hybrid_${TIMESTAMP}.json
sync; sleep 5
...
PYTHONWARNINGS=ignore python ${BENCH} \
    ...
    --json results_C_4t_serial_${TIMESTAMP}.json
sync; sleep 5
...
PYTHONWARNINGS=ignore mpiexec -n $RANKS_HEAVY python ${BENCH} \
    ...
    --json results_C_4t_npool_${TIMESTAMP}.json
sync; sleep 5
...
PYTHONWARNINGS=ignore mpiexec -n $RANKS_HEAVY python ${BENCH} \
    ...
    --json results_C_4t_hybrid_${TIMESTAMP}.json
sync; sleep 5
...
for tag, desc in variants:
    ...
    for file_tag, mode_name in modes:
        path = f"results_C_{tag}_{file_tag}_{ts}.json"
        w = read_wall(path)
        walls[mode_name] = w
        if w is not None:
...
echo "JSON files: results_C_*_${TIMESTAMP}.json"
