...
def pool_starmap(pool, func, iterable):
    """Run `starmap` on `pool`, or plain sequential calls when `pool` is None.

    Lets the same code work with or without a process pool. With a pool the
    work is spread over processes; without one it just runs here, one item
    after another. Same inputs, same outputs, same order either way.

    Parameters
    ----------
    pool : Pool or None
        Process pool to use, or `None` to run sequentially.
    func : callable
