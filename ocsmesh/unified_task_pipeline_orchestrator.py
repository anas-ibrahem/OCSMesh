...
# Safely import HPC Dynamic Runner if available locally
try:
...
def compute_prime_factors(payload: dict) -> dict:
    """Sample Task 1: Factorize a number."""
...
def compute_simulation(payload: dict) -> dict:
    """Sample Task 2: Data simulation."""
...
class JobProcessor:
    """
    A unified processing facade that handles task accumulation, 
    method registration, and backend execution internally.
    
    The user simply invokes 'do_a()', 'do_b()', etc., and retrieves 
    results via 'get_output()'.
    """
    def __init__(self, mode: str = "serial"):
        ...
        # Internal worker registry mapping method keys to user functions
        self.worker_registry: Dict[str, Callable] = {
            'factorize': compute_prime_factors,
            'simulate': compute_simulation
        }
    ...
    def _execute_single_task(self, task: TaskSpec) -> Any:
        ...
        if isinstance(self.worker_registry, dict) and action in self.worker_registry:
    ...
    def _process_mpi(self, tasks: List[TaskSpec]) -> Optional[List[Any]]:
        ...
        Instantiates and executes HPCDynamicTaskRunner entirely under the hood.
        """
...
if __name__ == "__main__":
    # 1. Instantiate orchestrator facade specifying backend ('serial', 'multiprocessing', or 'mpi')
    execution_mode = "mpi"
    pipeline = JobProcessor(mode=execution_mode)

    # 2. Queue workload by calling method actions directly on the pipeline object
    # No TaskSpec lists, no dictionary registries, no runner management!
    ...
    # 4. Print clean execution report on Rank 0
